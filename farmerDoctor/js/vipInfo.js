	...
	//判断一下鼠标离开谁了 
		if($(this).is("#inputUser")){ 
			if(!/^[a-zA-Z0-9_-]{6,20}$/.test(this.value)){
				...
				$(this).next().text("");
				$(this).next().append("<img style='width: 18px;position: absolute;top: 172px;' src='img/勾 (1).png' />");
			} 
		} 
		if($(this).is("#inputMail")){ 		
			...
			if(!reg.test(this.value)){ 
				...
				$(this).next().text("");
				$(this).next().append("<img style='width: 18px;position: absolute;top: 272px;' src='img/勾 (1).png' />");
			} 
		} 
		...
		//	var $phone = $("#inputPhone").val(); 
			if(!reg.test(this.value)){ 
				...
				$(this).next().text("");
				$(this).next().append("<img style='width: 18px;position: absolute;top: 222px;' src='img/勾 (1).png' />");
			} 
		}
		if($(this).is("#inputBirthday")){ 		
			...
			if(!reg.test(birthday)){ 
				...
				$(this).next().text("");
				$(this).next().append("<img style='width: 18px;position: absolute;top: 322px;' src='img/勾 (1).png' />");
			} 
		} 
	}) 
})
	...
	$.ajax({
		...
		success:function(json){
			if(json.code==2000){
				...
				$("#sex-select").val(json.data.user.gender);
